...
const ICONS = {
  alert: (
    <HiBellAlert className='h-5 w-5 text-gray-600 group-hover:text-[#229ea6]' />
  ),
  message: (
    <BiSolidMessageRounded className='h-5 w-5 text-gray-600 group-hover:text-[#229ea6]' />
  ),
};
                  ...
                  <div className='grid grid-cols-2 divide-x bg-gray-50'>
                    {callsToAction.map((item) => (
                      <Link
                        ...
                        className='flex items-center justify-center gap-x-2.5 p-3 font-semibold text-[#229ea6] hover:bg-gray-100'
                      >
                        {item.name}
                      </Link>
                    ))}
